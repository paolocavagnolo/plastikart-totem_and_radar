...
void goPlay() {
  ...
  if ((micros() - frameT) > uspf) {
    ...
    if (frame >= frameCOUNT) {
      while (!digitalRead(play_s) or !digitalRead(dmx_pin)) {
        kill();
        digitalWrite(play_led,HIGH);
        delay(50);
        digitalWrite(play_led,LOW);
...
void modeState() {
  ...
  if ((!digitalRead(play_s) or !digitalRead(dmx_pin)) and !rec) {
    play = true;
    digitalWrite(play_led, HIGH);
    deB = millis();
  }
...
void writeToSd() {
  ...
  if (SD.exists("mt1.txt")) {
    SD.remove("mt1.txt");
  }
  myFile = SD.open("mt1.txt", FILE_WRITE);
  for (int i=0; i<frameCOUNT; i++) {
    myFile.println(mt1[i]);
  }
  myFile.close();

  if (SD.exists("mt2.txt")) {
    SD.remove("mt2.txt");
  }
  myFile = SD.open("mt2.txt", FILE_WRITE);
  for (int i=0; i<frameCOUNT; i++) {
    myFile.println(mt2[i]);
  }
  myFile.close();

  if (SD.exists("lux.txt")) {
    SD.remove("lux.txt");
  }
  myFile = SD.open("lux.txt", FILE_WRITE);
  for (int i=0; i<frameCOUNT; i++) {
    myFile.println(lux[i]);
  }
  myFile.close();
...
void writeBAKToSd() {
  ...
  if (SD.exists("mt1.bak")) {
    SD.remove("mt1.bak");
  }
  myFile = SD.open("mt1.bak", FILE_WRITE);
  for (int i=0; i<frameCOUNT; i++) {
    myFile.println(mt1[i]);
  }
  myFile.close();

  if (SD.exists("mt2.bak")) {
    SD.remove("mt2.bak");
  }
  myFile = SD.open("mt2.bak", FILE_WRITE);
  for (int i=0; i<frameCOUNT; i++) {
    myFile.println(mt2[i]);
  }
  myFile.close();

  if (SD.exists("lux.bak")) {
    SD.remove("lux.bak");
  }
  myFile = SD.open("lux.bak", FILE_WRITE);
  for (int i=0; i<frameCOUNT; i++) {
    myFile.println(lux[i]);
  }
  myFile.close();
...
void readFromSd() {
  ...
  int i=0;
  if (SD.exists("mt1.txt")) {
    myFile = SD.open("mt1.txt");
    while (myFile.available()) {
      mt1[i] = myFile.parseInt();
      i++;
    }
    myFile.close();
    frameCOUNT = i;
  ...
  i=0;
  if (SD.exists("mt2.txt")) {
    myFile = SD.open("mt2.txt");
    while (myFile.available()) {
      mt2[i] = myFile.parseInt();
      i++;
    }
    myFile.close();
    frameCOUNT = i;
  ...
  i=0;
  if (SD.exists("lux.txt")) {
    myFile = SD.open("lux.txt");
    while (myFile.available()) {
      lux[i] = myFile.parseInt();
      i++;
    }
    myFile.close();
    frameCOUNT = i;
